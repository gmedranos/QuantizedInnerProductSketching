...
class PSSketch(InnerProdSketch):
    def __init__(self, sk_hashes: np.ndarray, sk_values: np.ndarray, tau: float, norm: int, vec_len: int) -> None:
        self.sk_hashes: np.ndarray = sk_hashes
        self.sk_values: np.ndarray = sk_values
        self.tau: float = tau
        self.norm: int = norm
        self.vec_len: int = norm
    ...
    def get_vector(self):
        ...
        vec[self.sk_hashes] = self.sk_values
        vec_weighted = np.where(vec == 0, 0, vec / np.minimum(1, vec ** 2 * self.tau))
        return vec_weighted

class PS(InnerProdSketcher):
    def __init__(self, sketch_size: int, seed: int, norm : int = 2) -> None:
    ...
    def sketch(self, vector: np.ndarray) -> PSSketch:
        ...
        vec_len = len(vector)
        # From sketch size in bits to sketch size in samples
        self.sketch_size = int(self.sketch_size // (np.log2(len(vector)) + 16))
        try:
        ...
        return PSSketch(indices_under_tau, sk_values, tau, self.norm, vec_len)